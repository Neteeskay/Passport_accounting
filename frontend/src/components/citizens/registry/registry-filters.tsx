...
export function RegistryFilters({ filters, onChange }: RegistryFiltersProps) {
  ...
  return (
    <div className="rounded-[14px] border border-border bg-card p-4">
      <div className="grid grid-cols-[minmax(190px,1fr)_104px_132px_150px_170px] items-start gap-2">
        <FilterField className="space-y-1" label="Поиск">
          <div className="relative">
            <Search className="absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-muted-foreground" />
            <input
              className="h-9 w-full rounded-[18px] border border-border bg-background pl-11 pr-4 text-[13px] text-foreground outline-none placeholder:text-muted-foreground focus:border-primary/45 focus:ring-2 focus:ring-primary/15"
              value={filters.query}
        ...
        <FilterField className="space-y-1" label="Пол">
          <select
        ...
        <FilterField className="space-y-1" label="Паспорт">
          <input
        ...
        <FilterField className="space-y-1" label="Адрес">
          <input
            className="h-9 w-full rounded-[18px] border border-border bg-background px-3 text-[13px] text-foreground outline-none placeholder:text-muted-foreground focus:border-primary/45 focus:ring-2 focus:ring-primary/15"
