...
- admin-only endpoint-ы для создания, редактирования и удаления пользователей системы.
...
- `admin` может создавать, редактировать и удалять пользователей через `POST/PUT/DELETE /api/v1/auth/users...`
